...
export const AvatarGroup: React.FC<AvatarGroupProps> = ({
  ...
  if (isLoading) {
    return (
      <div className="flex items-center -space-x-4 mt-3 animate-pulse">
  ...
  return (
    <div
      className="flex items-center -space-x-4 mt-3 cursor-pointer"
      onClick={() => {
        clearActiveUser();
    ...
    >
      {visibleUsers.length === 0 ? (
        <div
          className={`relative inline-flex items-center justify-center rounded-full font-bold border-2 border-slate-900 shadow-md bg-gray-800 text-white transition-transform hover:scale-105 ${sizeClasses[size]}`}
